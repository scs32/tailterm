...
- Tabs with unread output pulse their existing background fill; “New output” stays in the hover details and browser title. Reduced-motion preferences use a steady tint. **Bell**, **Needs attention**, and **Command finished** retain their text indicators. Completion requires the remote shell to emit an OSC 133 command-finished marker; silence is not treated as completion. Selecting a tab clears its indicator.
...
Automatic local scrolling is enabled by default for connected tmux sessions. Scroll upward to open the colored local snapshot; scroll downward past its bottom to return to live. Wheel movement during loading is carried into the snapshot, and trailing momentum is absorbed at the handoff. Hold Shift to use remote scrolling, or turn off **Automatic local scrolling** in Appearance to use manual mode. The small **Scroll off / Scroll on** button remains available in either mode. Each capture fetches a local snapshot of up to 5,000 history lines plus the visible pane, bounded to 4 MiB including formatting. The read-only terminal viewer preserves ANSI and true-color foreground/background colors, bold, underline, and inverse formatting. It uses the live terminal’s font, theme, spacing, and contrast correction, and only renders the visible rows (with a 20,000-row local scrollback limit after wrapping). Find searches the snapshot, Copy uses its selected text, and Command/Ctrl-click opens web URLs. Scrolling, search and typing in this view send no input to SSH. Capture never enters tmux copy mode or changes its mouse setting. Only text and SGR styling are replayed; terminal controls, clipboard/title requests and other escape commands are removed. Click the toggle again or press Escape to return to live. The snapshot stays fixed while open and is discarded on close, disconnect, or lock; it is not written to browser storage.
...
Open a tab’s **... → Tab appearance** to set a personal label, emoji, outline color, background fill, and typeface. A custom label replaces the server name and numbered suffix; clearing it restores the usual `Server #1` label. The outline color covers the complete tab frame and its controls, including hover and keyboard focus. Fill colors are tinted to keep text readable in light and dark themes. **Choose an emoji** opens a searchable local picker; you can also type or paste an emoji or symbol. Changes are saved in the encrypted workspace and survive reloads. A group has its own independent appearance, editable through **Group appearance**. Each session retains its original decoration when grouped or separated; **Focused session appearance** edits that session alone. Inactive pane headers keep their session’s colored underline. The active session has a full outline in that same session color. Group appearance is also saved in the encrypted workspace.
