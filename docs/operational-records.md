...
normalized operational records for the later consumer #3694. Owner requirement:
...
Release order **#4074** deployed the accepted source
`3da7ae703ac6528e9aeb2151b03dcc4335f8e2ec` to the TrueNAS hub and Mini CLI.
The immutable [operator receipt](releases/operational-records-3da7ae7/release-receipt.json),
its 15 nested evidence files and the separate
[handler-saved acceptance](releases/operational-records-3da7ae7/acceptance.json)
are retained without rewriting the receipt's original null acceptance and
...
Automatic continuation, scheduling, complete current-state/backlog sweeps,
deterministic GO, verified wake transport, MCP adapters, UI, AIV integration,
cryptographic/external execution attestation, Air CLI/frontend support and task
cleanup remain outside the deployed slice. Consumer #3694/#4053 remains a
separate follow-through implementation. The schema addition retained existing
data; both new operational-record tables were empty after migration, retained
profile hashes matched, integrity was OK and foreign-key violations were zero.
Never restore an old database to roll back; older binaries do not enforce this
capability and cannot be presented as supporting committed operational records.
