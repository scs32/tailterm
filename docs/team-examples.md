...
board, assigns bounded work, integrates findings, and owns the final result.
These are LLM instructions, not a guarantee of compliance or a blocking handshake.
...
| Build and review | 2 | builder | A focused feature, bug fix, or refactor that benefits from a second set of eyes. |
...
While waiting, do useful independent inspection within your role. When none remains, send the precise dependency, mark your turn done with a waiting explanation, and end the turn; a later directed message can resume you. Do not busy-poll, repeatedly ask the owner, or declare the whole task complete. Only spawn helpers for a concrete independent assignment when task settings permit it; honor the shared Max new agents allowance. No recursive delegation for its own sake. After the orchestrator accepts your final handoff and releases you, use tt retire and finish quietly. Orchestrators use tt retire NAME for workers no longer needed, after checking review and dependency needs; at task completion retire all workers and helpers while remaining available to the owner. Use tt resume NAME before assigning more work to a retired member. Do not kill tmux sessions for routine cleanup.
...
**Workflow:** Lead defines contract and file ownership; UI/API work in parallel; QA verifies the integrated result.
...
### fixer — Diagnosis lead and repair
...
### analyst — Independent causal analysis
...
**Workflow:** Introductions → orchestrator assigns owned lanes → workers share evidence → independent cross-check → orchestrator integrates and reports.
...
### orchestrator — Main orchestrator and integration
