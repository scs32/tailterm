...
| `GET /v1/tasks/{task}/message-audit/changes` | Project-scoped, ordered, frozen audit-event feed. Optional `kind=intake|work`; continuation cursor is bound to project, filter, cutoff, and position. |
...
The A2 tests cover original/current separation, frozen posting receipt replay, complete correction history, old-message correction without a new message, count/filter/byte-bounded frozen cursors, malformed and swapped cursors, Intake-to-existing and Intake-to-new resolution, native-create receipt agreement in both directions, same-key and competing races, injected transaction failures, related/duplicate/primary validation, current and retained item revisions, explicit human/handler and dispatch-derived foreign authority, arbitrary foreign rejection, exact active-run enforcement, recovery after retirement/closure, additive schema, reopen idempotence, and foreign-key integrity.

An executable compatibility check built the exact old hub at `c7730738611555bd5474ceca166a23c239ca1690` and the A2 hub separately. The new binary initialized a temporary A2 database; the old binary then opened it and posted a real A1-linked message over loopback HTTP; the new binary reopened it and exposed `work` revision 1 while the old posting receipt still returned the frozen original. In the final pre-commit run, the old-binary SHA-256 was `998ec2fb770ff16deb4dda13443a4a1e990cc4a9b959b427a30e997743d0c075`; the candidate-binary SHA-256 was `592f72e797809b1b9dd80e59c638d10cb5e03b7b6c4296d308c41c206513d2b7`.
