...
At candidate handoff, lead review and database-handler revision-checked result storage were still required before A2 acceptance, and deployment required a separate recorded release order. Candidate implementation performed no live historical correction manifest, production backfill, required-mode switch, Board/cache/export integration, AIV work, or deployment. Lead acceptance and the later release order are recorded below. The shared preview service remained independent and untouched.

## Accepted release stage

Lead accepted corrected candidate `b7dd55d7190cf1932ef4cd13ea3bd883dd440276` in Board message 1445 and issued the separate hub-only release order in message 1446. Candidate commits were integrated conflict-free from exact clean root `c7730738611555bd5474ceca166a23c239ca1690`; application/source HEAD is `eba02aa935ecbb1cd696ff970300e7a5e2e458d5`. Exact clean application source is retained at `.build/releases/message-audit-a2-eba02aa`.

The prescribed static Linux amd64 hub build is 26,960,034 bytes with SHA-256 `16786368ccd79221df58001a65b88894fd1f99611b9ba76c2c7f25dd3bf1fa01`. Before update, the required rollback binary `/mnt/deepfreeze/tailterm-hub/releases/20260909-narrative-history-4cb6e6c/tailterm-hub` matched SHA-256 `49591cd6083a590c0afca68c05f50ba416c1e98f01ae006e92bb341e956b5475`. A consistent SQLite online backup was created through Python's SQLite backup API at `/mnt/deepfreeze/tailterm-hub/backups/before-message-audit-a2-20260910T134441Z.sqlite`; it is mode 0600, 9,977,856 bytes, SHA-256 `4d68adfb62a0873c4990d977b4106f4ec27f2c65cbca46becdb51e49e8065012`, integrity OK, with zero foreign-key violations.

The source-inspected middleware path deployed only unique release `20260910-message-audit-a2-eba02aa`. Post-update evidence shows exactly one RUNNING `hub` container using the existing distroless image, `100.116.238.37:18765` TCP binding, state mount, and read-only token/binary mounts. The deployed binary matches the local build exactly. Live database integrity remains OK with zero foreign-key violations and exactly the seven expected additive `message_audit_*` tables. No Tailscale, network, service topology, token, live work-item, profile, historical correction, backfill, feature/status, AIV, or frontend state was changed or used as a test fixture.

Existing installed Mini and Air CLIs remain byte-identical at SHA-256 `a2a640d18e3d3014bcd8afdafbaf1f8994c5fbaf4623ea168014aeba7f68b49a`; their relay PIDs remain 83457 and 912. Authenticated existing-CLI doctor and agent reads pass against the updated hub, while unauthenticated whoami remains 403. The TailOS and Mini `release.json` bodies remain byte-identical to dropdown frontend manifest SHA-256 `9280148ec37f1dcbbf51300ed04f973597abfeb7956da3bfdb21c4380480b17e`, application commit `8d47e9b4b6c34cc1b9331517ba893156d28e23e8`; detached preview PID 60799/PPID 1 remains the listener and was not restarted. No frontend, WASM, Cloudflare, Mini package, Air preview, CLI binary, or relay was deployed.

Normal rollback uses TrueNAS middleware to point the app at the exact retained narrative binary while keeping the additive current database. Old-binary A1 writes are imported only when A2 is opened again. The online backup is disaster-recovery evidence; restoring it would lose all writes after 2026-09-10T13:44:41Z. The machine-readable receipt is `docs/releases/tailos-2026-09-10-message-audit-a2.json`. A2 stage acceptance and database-handler report storage remain required; the parent roadmap remains in progress with B1/B2/C1/E1/E2 pending.
