...
The combined source adds a database-handler-only `tt work-items evidence`
collector with explicit current, revision, full linked-message and keyed
update-receipt source selection. Native pages are followed to terminal metadata
and retained byte-for-byte with private size/hash manifests; exact retries resume
after the last valid cursor. A current-item revision/scope bookend and per-page
coverage checks reject changed or mixed observations.
