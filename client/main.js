...
function mount() {
  ...
  if (staticMode) {
    ...
    boardView = createBoardView({
      ...
      settings: (id) => taskHub.settings(id),
...
function renderTabs() {
  ...
  $("#terminal-status").textContent = t
    ? `${t.status === "Connected" ? "●" : "○"} ${t.status} · ${t.server.username}@${t.server.host} · ${t.transport === "browser" ? "SSH over Tailscale" : t.transport === "wasm" ? "Tailscale WASM" : "SSH"}${t.tmux ? " · tmux launch: " + t.session + (t.tmuxVerified ? "" : " (unverified)") : ""}`
