...
export function createWorkItemsView({
  ...
  function render() {
    root.innerHTML = `<div class="work-items-view"><div class="work-items-head"><h2>${plural} <span class="count-badge">${items.length}</span></h2><div class="work-items-controls"><label>Project<select data-items-project><option value="">All projects</option>${tasks.map((t) => `<option value="${esc(t.id)}" ${scope === t.id ? "selected" : ""}>${esc(t.name)}${t.status === "closed" ? " · closed" : ""}</option>`).join("")}</select></label><label>Status<select data-items-status><option value="">All statuses</option>${options(statuses, state)}</select></label><button data-items-new class="primary" ${!tasks.some((t) => t.status === "open") || project(scope)?.status === "closed" ? "disabled" : ""}>＋ New ${singular.toLowerCase()}</button></div></div><div class="work-items-list">${items.map((item) => `<article class="work-item" data-work-item="${esc(item.id)}"><div class="work-item-copy"><button class="work-item-title" data-item-edit="${esc(item.id)}">${esc(item.title)}</button><span class="fine">${esc(project(item.taskId)?.name || item.taskId)} · ${esc(statuses[item.status])} · ${esc(priorities[item.priority])}${item.lastDispatch ? ` · Sent to ${esc(project(item.lastDispatch.targetTaskId)?.name || item.lastDispatch.targetTaskId)}` : ""}</span></div><button data-item-send="${esc(item.id)}" ${project(item.taskId)?.status === "closed" ? "disabled" : ""}>Send to project</button></article>`).join("")}</div></div>`;
