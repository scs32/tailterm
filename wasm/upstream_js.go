...
var testIPNFactory func(js.Value) any
...
func main() {
	...
	js.Global().Set("newIPN", js.FuncOf(func(this js.Value, args []js.Value) any {
		...
		if testIPNFactory != nil {
			return testIPNFactory(args[0])
		}
...
func (i *jsIPN) run(jsCallbacks js.Value) {
	...
	i.lb.SetNotifyCallback(func(n ipn.Notify) {
		...
		// Do not log identity-bearing notifications.
		if n.State != nil {
			notifyState(*n.State)
		}
		if n.SelfChange != nil {
...
func (i *jsIPN) ssh(host, username string, termConfig js.Value) map[string]any {
	...
	closeFn := js.FuncOf(func(this js.Value, args []js.Value) any { return jsSSHSession.Close() == nil })
	resizeFn := js.FuncOf(func(this js.Value, args []js.Value) any {
		rows, cols := args[0].Int(), args[1].Int()
		go jsSSHSession.Resize(rows, cols)
		return true
	})
	jsSSHSession.callbacks = []js.Func{closeFn, resizeFn}
	go jsSSHSession.Run()
	return map[string]any{"close": closeFn, "resize": resizeFn}
}
