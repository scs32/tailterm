...
const client={base:location.origin,token:'fixture',cacheStatus:()=>({label:'Isolated data'}),subscribe:()=>({stop(){}}),listTasks:async()=>[task],listWorkItems:async()=>({items:[item],next:0}),
 listWorkItemRevisions:async()=>({revisions:[{...item,itemId:item.id,revision:3,changeKind:'updated',provenance:'native',updatedBy:{user:'database-handler'},updatedAt:'2026-09-09T00:00:00Z',description:'Retained script-like text without execution'}],nextAfter:0}),
 listWorkItemHistoryGaps:async()=>({gaps:[],nextAfter:0}),listWorkItemMessages:async()=>({links:[{itemRevision:3,revisionCoverage:'verified',source:true,message:{seq:606,taskId:task.id,from:{user:'original-lead'},text:'Original <img src=x onerror=window.pwned=true> request',createdAt:'2026-09-08T00:00:00Z'}}],nextAfter:0}),
