...
func New(st *store.Store, identity Identity) *Server {
	...
	m.HandleFunc("GET /v1/team-queues", s.teamQueuesByHost)
	...
	m.HandleFunc("POST /v1/tasks/{id}/work-items/{wid}/order-scope/confirm", s.confirmWorkOrderScope)
	m.HandleFunc("POST /v1/tasks/{id}/work-items/{wid}/order-bookkeeping", s.saveWorkOrderBookkeeping)
	m.HandleFunc("GET /v1/tasks/{id}/work-items/{wid}/order-bookkeeping/receipts/{requestID}", s.getWorkOrderBookkeepingReceipt)
