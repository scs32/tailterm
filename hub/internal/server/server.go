...
func New(st *store.Store, identity Identity) *Server {
	...
	m.HandleFunc("POST /v1/tasks/{id}/required-deliveries", s.createRequiredDelivery)
	m.HandleFunc("POST /v1/tasks/{id}/operational-records", s.proposeOperationalRecord)
	m.HandleFunc("POST /v1/tasks/{id}/operational-records/{recordId}/commit", s.commitOperationalRecord)
	m.HandleFunc("GET /v1/tasks/{id}/operational-records/{recordId}", s.getOperationalRecord)
	m.HandleFunc("POST /v1/tasks/{id}/deliveries/{deliveryId}/follow-through/check", s.checkDeliveryFollowThrough)
	m.HandleFunc("POST /v1/tasks/{id}/deliveries/{deliveryId}/follow-through/report", s.reportDeliveryFollowThrough)
